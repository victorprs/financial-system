defmodule FinancialSystem.Money do
  ...
  def as_string(%Money{} = money) do
    ...
    cond do
      int_part == "0" -> "0." <> Integer.to_string(money.minor_units)
      true -> String.replace_prefix(Integer.to_string(money.minor_units), int_part, int_part <> ".")
    end
  end
  ...
  defp trim_precision(%Money{} = money) do
    if money.precision > money.currency.decimal_places do
      ...
      {:ok, new_money} = Money.new(int_part <> "." <> fract_part, String.length(fract_part), money.currency)
      new_money
    else
      money
    end
